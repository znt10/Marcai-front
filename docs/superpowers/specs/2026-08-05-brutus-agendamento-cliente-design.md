...
services:
  ...
  app:
    ...
    environment:
      DATABASE_URL: postgresql://brutus:brutus@db:5432/brutus
...
**`init-test-db.sql`.** Script de inicialização que cria o banco `brutus_test` na primeira subida do volume, para os testes de integração (§12) não sujarem o banco de desenvolvimento. Roda **uma única vez**, quando o volume `pgdata` é criado; recriar o banco de teste depois exige `docker compose down -v`.
...
**`Barbearia`** — linha única, criada pelo seed.

| Campo | Tipo | Nota |
|---|---|---|
| `id` | String @id | fixo `"brutus"` |
| `nome` | String | `"BRUTUS"` |
...
| `whatsapp` | String @unique | dígitos apenas; vira o login na Etapa 2 |
...
| `id` | String @id @default(uuid()) | |
| `nome` | String | |
| `whatsapp` | String @unique | dígitos apenas, com DDD |
| `criadoEm` | DateTime @default(now()) | |
...
|---|---|---|
| `id` | String @id @default(uuid()) | |
| `codigo` | String @unique | 10 caracteres, URL pública |
...
### 5.2 Código público do agendamento
...
### 5.3 Restrição de exclusão (proteção contra sobreposição)
...
Reproduz o cenário do wireframe:
...
> As rotas públicas **nunca** retornam nome de cliente, telefone de cliente, motivo de bloqueio, nem quais horários estão ocupados. Retornam exclusivamente o que está livre.
...
1. Cabeçalho `BRUTUS · barbearia`, subtítulo com endereço e `horarioResumo`
...
Roda contra `DATABASE_URL_TEST` (banco `brutus_test`, criado pelo `init-test-db.sql` — §2.1). Basta `docker compose up -d db`; não precisa do serviço `app` no ar. Cada teste limpa as tabelas num `beforeEach`, para a ordem de execução não importar.
...
```
# Dentro do Docker Compose o host é `db`; da máquina, `localhost:5433` (§2.1)
DATABASE_URL="postgresql://brutus:brutus@db:5432/brutus"
DATABASE_URL_HOST="postgresql://brutus:brutus@localhost:5433/brutus"
DATABASE_URL_TEST="postgresql://brutus:brutus@localhost:5433/brutus_test"
